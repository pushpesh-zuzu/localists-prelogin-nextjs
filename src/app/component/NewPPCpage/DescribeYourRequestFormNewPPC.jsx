...
const DescribeYourRequestFormNewPPC = () => {
    ...
    const { requestId, qualityData, addDetailLoader, buyerRequest } = useSelector(
        (state) => state.buyer
    );
    ...
    useEffect(() => {
        if (buyerRequest?.notes && text === "") {
            setText(buyerRequest.notes);
        }
    }, [buyerRequest?.notes]);
...
export default DescribeYourRequestFormNewPPC;
